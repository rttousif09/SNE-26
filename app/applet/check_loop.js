...
for (const file of files) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (insideUseEffect) {
      ...
      // Look for setXYZ inside useEffect
      if (line.match(/set[A-Z][a-zA-Z0-9]*\(/)) {
         // console.log(`Found setState in useEffect: ${file}:${i+1} -> ${line.trim()}`);
      }
      
      if (useEffectBraces <= 0) {
        insideUseEffect = false;
        
        // check the dependency array line (current or next)
        let depLine = line;
       ...
       // Look for state setters OUTSIDE useEffect and OUTSIDE handlers
